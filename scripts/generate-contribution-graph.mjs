...
weeks.forEach((days, weekIndex) => {
  days.forEach((day) => {
    ...
    cells.push(`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" rx="3" fill="${color}" stroke="#30363d" stroke-width="1"><title>${title}</title></rect>`);
  });
});
...
weeks.forEach((days, index) => {
  ...
  if (month !== previousMonth) {
    labels.push(`<text x="${left + index * (cell + gap)}" y="34" class="label">${month}</text>`);
    previousMonth = month;
  }
});
...
const svg = [
  ...
  `  <style>.title{fill:#f0f6fc;font:700 18px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.label{fill:#8b949e;font:12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.meta{fill:#8b949e;font:12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}</style>`,
  `  <rect width="${width}" height="${height}" rx="12" fill="#0d1117"/>`,
  `  <text x="${left}" y="24" class="title">Contribution Graph</text>`,
  `  ${labels.join("\n  ")}`,
  `  <text x="10" y="88" class="label">Mon</text>`,
  `  <text x="10" y="122" class="label">Wed</text>`,
  `  <text x="10" y="156" class="label">Fri</text>`,
  `  ${cells.join("\n  ")}`,
  `  <text x="${left}" y="${height - 18}" class="meta">${escapeXml(subtitle)} - refreshed hourly</text>`,
  `</svg>`,
  "",
].join("\n");
